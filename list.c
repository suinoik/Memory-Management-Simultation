...
void list_add_ascending_by_address(list_t *l, block_t *newblk) { // done!

  /*
   * 1. Insert newblk into list l in ascending order based on the START address
   * of the block.
   *
   *    node_t *c = l.head;
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk) { // done!
  /*
...
void list_coalese_nodes(list_t *l) { // pending
  /*
   * 1. Assuming you have passed in a sorted list of blocks based on addresses
   * in ascending order
   * 2. While list is not empty,
   *    a. compare two nodes at a time to see if the prev.END + 1 ==
   * current.START, if so, they are physically adjacent combine them by setting
   * the prev.END = current.END. b. If not adjacent go to #6
...
bool list_is_in_by_pid(list_t *l, int pid) { // done!
