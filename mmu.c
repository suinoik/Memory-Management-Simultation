...
void allocate_memory(list_t *freelist, list_t *alloclist, int pid,
   ...
   * 2. if so, remove it and go to #3, if not print ""Error: Memory Allocation
   * <blocksize> blocks\n""
...
void deallocate_memory(list_t *alloclist, list_t *freelist, int pid,
   ...
   * 2. if so, remove it and go to #3, if not print "Error: Can't locate Memory
   * Used by PID: <pid>"
...
list_t *coalese_memory(list_t *list) {
  ...
  while ((blk = list_remove_from_front(list)) !=
         NULL) { // sort the list in ascending order by address
...
int main(int argc, char *argv[]) {
  ...
  list_t *FREE_LIST =
      list_alloc(); // list that holds all free blocks (PID is always zero)
